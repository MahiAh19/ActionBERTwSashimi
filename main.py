...
def main():
    parser = argparse.ArgumentParser(
        description='Video + QA setup for Social-IQ')

    # Experiment params
    parser.add_argument('--mode',           type=str,
                        help='train or test mode', required=True, choices=['train', 'test'])
    parser.add_argument('--expt_dir',       type=str,
                        help='root directory to save model & summaries', required=True)
    parser.add_argument('--expt_name',      type=str,
                        help='expt_dir/expt_name: organize experiments', required=True)
    parser.add_argument('--run_name',       type=str,
                        help='expt_dir/expt_name/run_name: organize training runs', required=True)
    ...
    parser.add_argument('--use_pretrained', type=str2bool,
                        help='use pre-trained transformer', default='true')
    parser.add_argument('--num_layers',     type=int,
                        help='no. of layers in the RNN/Transformer', default=1)
    parser.add_argument('--num_cls',        type=int,
                        help='no. of class labels', default=101)
    parser.add_argument('--model_ckpt',     type=str,
                        help='resume train / perform inference; e.g. model_100.pth')

    # Data params
    parser.add_argument('--data_dir',       type=str,
                        help='root dir containing all delete files', required=True)
    parser.add_argument('--pred_output',    type=str,
                        help='prediction file (label, pred) pair on each line')
    ...
    parser.add_argument('--log_interval',   type=int,
                        help='interval size for logging training summaries', default=100)
    parser.add_argument('--save_interval',  type=int,
                        help='save model after `n` weight update steps', default=1000)
    ...
    parser.add_argument('--gpu_id',         type=int,
                        help='cuda:gpu_id (0,1,2,..) if num_gpus = 1', default=0)
    parser.add_argument('--opt_lvl',        type=int,
                        help='Automatic-Mixed Precision: opt-level (O_)', default=1, choices=[0, 1, 2, 3])
    # parser.add_argument('--num_gpus',    type=int,   help='number of GPUs to use for training', default=1)

    # Misc params
    parser.add_argument('--num_workers',    type=int,
                        help='number of worker threads for Dataloader', default=1)
